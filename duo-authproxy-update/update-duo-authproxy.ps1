...
Function Repair-CommentedSecrets() {
    ...
    try {
        ...
        $newConfigContent = Get-Content -Path "$ConfigFileLocation\$TargetConfigFilename" | `
            Where-Object { $_ -notmatch '^;.*(skey|secret|password).*=.*\S+' }
...
Function Repair-UpdateNeeded() {
    ...
    if ($PerchServices) {
        try {
            ...
            Write-warn -message 'Failed to stop running Perch service(s).'
            write-warn -message $_ -level 2
        }
    }
    ...
    if (get-service -name $TargetServiceName | Where-Object {$_.Status -eq "Running"}) {
        ...
        try {
            ...
            Write-warn -message "Failed to stop $TargetServiceName."
            write-warn -message $_ -level 2
        }
    } else {
    ...
    $FileLocked = $false
    try {
        $FileCheck = [System.IO.File]::Open("$configFileLocation\..\bin\servicemanager.pyd",'Open','Write')
        $FileCheck.Close()
        $FileCheck.Dispose()
    } catch {
        $FileLocked = $true
    ...
    if ($FileLocked) {
        ...
        try {
            ...
            Write-warn -message 'AuthProxy update failed.'
    ...
    if ($DuoServiceRunning) {
        try {start-service -name $TargetServiceName -WarningAction SilentlyContinue}
            ...
            Write-warn -message "$TargetServiceName failed to start."
            write-warn -message $_ -level 2
        }
    } else {
        Write-warn -message 'Duo service was not already running. Opting not to try starting it.'
    }
    ...
    if ($PerchServices) {
        try {start-service $PerchServices}
            ...
            Write-warn -message 'Perch service(s) failed to start.'
            write-warn -message $_ -level 2
        }
    }
...
Function Restart-DuoService() {
    #Restart the Duo AuthProxy service depending on it's current status
  
    #Get the current status of the Duo service
    $DuoServiceStatus = (get-service -name $TargetServiceName).status
  
    #Assume restart will not be needed after this function runs
    $ServiceRestartStillNeeded = $false
  
    #Restart the service or leave it alone if it was not already running
    switch ($DuoServiceStatus) {
        'Running' {
            try {
                restart-service -name $TargetServiceName -WarningAction SilentlyContinue -Force
                write-ok -message 'Duo service successfully restarted.'
  
                $ServiceRestartStillNeeded = $false   #Restart no longer needed if this was successful
            } catch {
                Write-warn -message "$TargetServiceName service failed to start."
                write-warn -message $_ -level 2
  
                $ServiceRestartStillNeeded = $true   #Restart still needed on failure
            }
        } 'Stopped' {
            Write-warn -message 'Duo service was not already running. Opting not to try starting it.'
  
            $ServiceRestartStillNeeded = $false   #Restart not needed if the service was not already running
        } default {
            Write-warn -message "Duo service status is $DuoServiceStatus. Opting not to try restarting it."
  
            $ServiceRestartStillNeeded = $true   #Restart still needed as no changes were made
        }
    } #End of switch block
  
    return $ServiceRestartStillNeeded
}
...
if ($AutoFixApplies) {
    ...
    if ($RemediationNeeded['UpdateNeeded'] -and $AutoFix['UpdateNeeded']) {
        $RemediationNeeded['UpdateNeeded'] = Repair-UpdateNeeded($ConfigFileLocation)
    }
